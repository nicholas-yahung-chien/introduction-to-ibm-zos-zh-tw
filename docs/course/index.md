# 課程首頁

<div class="course-dashboard">
  <div class="course-lede">
    這是 IBM 授權課程 <strong>Introduction to IBM z/OS</strong> 的台灣繁體中文化靜態教材。內容整理課程影片、中文字幕、繁體中文摘要、閱讀材料與 IBM Z 詞彙表，適合第一次接觸大型主機與 z/OS 的學習者。
  </div>
  <div class="course-stats">
    <div class="course-stat"><strong>18</strong><span>課程影片</span></div>
    <div class="course-stat"><strong>3</strong><span>主要單元</span></div>
    <div class="course-stat"><strong>3</strong><span>閱讀頁</span></div>
    <div class="course-stat"><strong>54</strong><span>互動練習</span></div>
  </div>
</div>

## 建議學習方式

1. 先閱讀「課程概觀」，了解 IBM Z 與 z/OS 在企業核心系統中的定位。
2. 依序完成「大型主機環境」、「大型主機基礎架構」與「大型主機安全」三個單元。
3. 每支影片先看完一次，再回到影片下方摘要確認學習重點與重要術語。
4. 遇到不熟悉的 IBM Z 名詞時，搭配詞彙表查閱原文術語與繁體中文說明。
5. 完成單元後到「互動練習」確認概念，答錯時回到對應影片複習。

## 課程單元

<div class="lesson-grid">
  <a class="lesson-card" href="./course-overview">
    <h3>課程概觀</h3>
    <p>課程介紹與學習目標。</p>
  </a>
  <a class="lesson-card" href="./mainframe-environment">
    <h3>大型主機環境</h3>
    <p>電腦架構、虛擬化、作業系統與 z/OS 的定位。</p>
  </a>
  <a class="lesson-card" href="./mainframe-infrastructure">
    <h3>大型主機基礎架構</h3>
    <p>IBM Z 元件、HMC、SE、Sysplex 與組態設定。</p>
  </a>
  <a class="lesson-card" href="./mainframe-security">
    <h3>大型主機安全</h3>
    <p>z/OS 安全、交易層級安全、Crypto Express、IOCDS 與 IPL。</p>
  </a>
  <a class="lesson-card" href="../practice/">
    <h3>互動練習</h3>
    <p>完成各單元後，透過即時回饋確認重點概念。</p>
  </a>
</div>

## 學完後你會理解

- 大型主機與一般伺服器在設計目標、可靠性與工作負載上的差異。
- z/OS 如何管理硬體資源、應用程式、資料存取、安全控制與系統啟動。
- LPAR、HMC、SE、Sysplex、IOCDS、Crypto Express 與 IPL 等入門術語的基本脈絡。

## 課程活動

<CourseManifest />
